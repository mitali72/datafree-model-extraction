...
class SwinTransformer3D(nn.Module):
    ...
    def inflate_weights(self, logger=None, sep_call=False):
...
if __name__ == '__main__':
    ...
    swint_backbone = SwinTransformer3D(embed_dim=96,
                          ...
                          patch_size=(2,4,4), 
                          window_size=(8,7,7), 
                          patch_norm=True).init_weights(pretrained='swin_tiny_patch244_window877_kinetics400_1k.pth')
    ...
    checkpoint = torch.load('swint_final_weights.pt')
    swint_classifier.load_state_dict(checkpoint)
    print('loaded weights')
    #swint_classifier.backbone.inflate_weights(sep_call=True)
    # print(swint_classifier.backbone)
    # print(swint_classifier.head)
    #print('inflated weights')
    ...
    bball = np.load('bb.npy')
    bball = torch.Tensor(bball)
    print(bball.size())

    result = swint_classifier(bball)
    print(result.size())
    print()
    '''
    bball_orig = np.load('snow.npy')
    bball = torch.Tensor(bball_orig[8]).unsqueeze(0)[:,:,0:10,:,:]
    print(bball.size())
    preds = swint_classifier(bball).to('cpu')
    b = torch.softmax(preds, 1)
    sorted, indic = torch.sort(b, 1)
    print(sorted.data[0][-5:])
    print(indic.data[0][-5:])
    print('----------------------------------------------------------------------------------------------------')

    # dummy = torch.rand(1, 3, 10, 224, 224)
    # print(dummy.shape)
